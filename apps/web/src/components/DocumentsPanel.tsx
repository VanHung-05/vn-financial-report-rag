    ...
    <div className="scrollbar-thin flex-1 overflow-y-auto p-4 md:p-6">
      <div className="mx-auto max-w-5xl">
        <div className="flex flex-wrap items-start justify-between gap-4">
          ...
          >
            {showForm ? "Đóng form" : "+ Thêm URL báo cáo"}
          </button>
        </div>
        ...
        {showForm && (
          ...
          >
            <p className="mb-3 text-sm font-medium text-ink-200">
              Thêm báo cáo bằng link PDF (Vietstock, IR công ty…)
            </p>
            <div className="grid gap-3 sm:grid-cols-2">
              <input
                ...
                placeholder="URL PDF *"
                className="sm:col-span-2 rounded-lg border border-ink-700 bg-ink-950 px-3 py-2 text-sm text-white"
              />
              <input
                value={form.title}
                onChange={(e) => setForm({ ...form, title: e.target.value })}
                placeholder="Tiêu đề"
                className="rounded-lg border border-ink-700 bg-ink-950 px-3 py-2 text-sm text-white"
              />
              <input
                value={form.ticker}
            ...
            >
              {submitting ? "Đang gửi…" : "Thêm & đưa vào hàng đợi"}
            </button>
          </form>
        )}
